...
import {IdrisAirdrop} from "src/IdrisAirdrop.sol";
import {IdrisToken} from "src/IdrisToken.sol";
import {Test} from "forge-std/Test.sol";
import {console} from "forge-std/console.sol";
import {DeployIdrisAirdrop} from "script/DeployIdrisAirdrop.s.sol";
import {ZkSyncChainChecker} from "lib/foundry-devops/src/ZkSyncChainChecker.sol";

contract MerkleAirdropTest is ZkSyncChainChecker, Test {
    IdrisAirdrop airdrop;
    IdrisToken token;
    address gasPayer;
    address user;
    uint256 userPrivKey;
    ...
    function setUp() public {
        if (!isZkSyncChain()) {
            DeployIdrisAirdrop deployer = new DeployIdrisAirdrop();
            (airdrop, token) = deployer.deployMerkleAirdrop();
        } else {
            token = new IdrisToken();
            airdrop = new IdrisAirdrop(merkleRoot, token);
